...
export default function WalletsPage() {
  ...
  useEffect(() => {
    api<{ wallets: string[] }>('/api/wallets')
      .then((d) => { setWallets(d.wallets); if (d.wallets.length) setActive(d.wallets[0]); })
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const d = await api<{ wallets: string[] }>('/api/wallets');
  ...
  return (
    <div className="space-y-8 max-w-5xl">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          {wallets.length === 0
            ? <option value="">No wallets loaded</option>
        ...
        <button
          ...
          disabled={!active}
          className="px-3 py-2 rounded-lg border border-mim-border text-mim-text-muted text-xs hover:border-mim-border-light hover:text-mim-text transition-colors disabled:opacity-40"
        >
          Unload
        </button>
        <button
          onClick={handleBackup}
          disabled={!active || backingUp}
          className="ml-auto px-3 py-2 rounded-lg border border-mim-border text-mim-text-muted text-xs hover:border-mim-border-light hover:text-mim-text transition-colors disabled:opacity-40"
        >
